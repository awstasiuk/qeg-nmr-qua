...
If its been a while since the last calibration, we must calibrate the wobb to maximize the signal from the resonator (see the :doc:`api/spectroscopy` for more details).
...
We can account for this detuning and phase drift by performing a simple free induction decay (FID) experiment.
...
c. The Q and auto-correlation will tell you how you must tune the offset-freq. 
   i. If the Q concavity is positive, offset-freq is too high. 
   ii. If Q concavity is negative, offset-freq is too low.
...
a. pulcal_1x.py, performs an amplitude sweep. It takes the pulse_amplitude from the config and scales it by the amp_list. 
   i. e.g. pulse_amplitude=0.441 Vpp and amp_list = np.arange(.93,1.05,.01) sweeps from 93% (0.410Vpp) to 105% (0.464Vpp) in increments of 1%.
   ii. It then runs the pulse once, and then runs it 4*n_wraps times again (for a pi/2 pulse, 4 pulses is approximately identity and "wraps" back around). Then it measures the amplitude of I.
...
a. Overrotation calibration: overrotcal_xy.py. This script will determine the necessary frame change, which can then be accounted for with :code:`expt.add_frame_change(angle, elements)` 